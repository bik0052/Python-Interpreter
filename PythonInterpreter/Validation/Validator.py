...
class Validator:
    ...
    def can_compile(file_path):
        ...
        try:
            ...
            module_ok = False
            print(f"Access Denied for the specified file {file_path}")
        return module_ok
    ...
    def execute_pep8(file_path):
        ...
        try:
            ...
            print(f'File at {file_path} is not found')
    ...
    def contains_multiple_class(source):
        result = True
        classes = [a_class.name for a_class in ast.walk(ast.parse(source))
                   if isinstance(a_class, ast.ClassDef)]
        if len(classes) > 1:
            result = False
        return result
